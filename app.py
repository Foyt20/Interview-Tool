...
if not st.session_state.setup_complete:
    ...
    with col2:
        st.session_state["position"] = st.text_input(label = "Position", max_chars = 40, value = st.session_state["position"], placeholder = "Enter the position")

        st.session_state["company"] = st.text_input(label = "Company", max_chars = 40, value = st.session_state["company"], placeholder = "Enter the Company")
...
if st.session_state.feedback_shown:
    ...
    feedback_completion = feedback_client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": """You are a helpful tool that provides feedback on interviewee performance.
                Before the feedback give a score of 1 to 10.
                Follow this format:
                Overall Score: //Your Score
                Provide a question by question score and feedback for each
